...
choices.forEach((choice, index) => {
  ...
  choiceImages[index].addEventListener("click", (e) => handleChoiceImageSelection(e, index));
});
...
function handleChoiceImageSelection(e, index) {
  if (!acceptingAnswers) return;
...
function showAlert(message, backgroundColor) {
  ...
  alertBox.style.lineHeight = "150px";
